(function (window, angular) {
  ...
  .value('software', {fakeData: false, timeout: 3000, app: {client_id: 'NTQ5NTE5MGViMTgzMDUw', name: 'ailegong', version: ''}, server: {address: 'http://www.tongshijia.com'}})
  .service('Base', Base)
  .service('Users', Users)
  ...
  function Offers($q,$log,Base,software,Users){
    ...
    function list(){
      var defer = $q.defer();
      Base.get('/api_orders/my_offers.json?access_token='+token.getTokenLocally().access_token).then(function(item){
        defer.resolve(item);    
      }, function(e){defer.reject(e)});
      return defer.promise;
    }
